...
class _JournalsPageState extends State<JournalsPage> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildTimeline(List<EventAnalysis> events) {
    ...
    sortedEvents.sort((a, b) {
      final timeA = TimeUtils.parseTimeRange(a.time_range);
      final timeB = TimeUtils.parseTimeRange(b.time_range);
    ...
    return Column(
      children: sortedEvents.asMap().entries.map((entry) {
        ...
        final timeRange = TimeUtils.parseTimeRange(event.time_range);
  ...
  Widget _buildTimelineEvent(EventAnalysis event, TimeOfDay time, bool isLast) {
    ...
    final endTime = TimeUtils.parseEndTime(event.time_range);
    final hasEndTime = TimeUtils.hasEndTime(event.time_range);
